...
class Tools_Swatch_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    public function getImageInfo($inputProduct) {
	...
	$bigImageHeight = intval($czoomHelper->getCfgGeneral('big_image_height'));
		...
		$product = Mage::getModel('catalog/product')->load($product->getId());
            $tmp['hi_res'] = Mage::helper('catalog/image')->init($product, 'image', $product->getImage())->__toString();
            $tmp['base_image'] = Mage::helper('catalog/image')->init($product, 'image', $product->getImage())->resize($baseImageWidth, $baseImageHeight)->__toString();
            $array[$product->getId()]['image'][] = $tmp;
			$images = $product->getMediaGalleryImages();

			foreach ($images as $_image) {
				$tmp['hi_res'] = Mage::helper('catalog/image')->init($product, 'image', $_image->getFile())->__toString();
				//$tmp['hi_res'] ='http://maximus.vn/media/catalog/product/cache/1/image/700x700/9df78eab33525d08d6e5fb8d27136e95/m/a/macrumors-ipad5b.jpg';
				$tmp['thumbnail'] = Mage::helper('catalog/image')->init($product, 'thumbnail', $_image->getFile())->resize(60, 60)->__toString();
				$tmp['base_image'] = Mage::helper('catalog/image')->init($product, 'image', $_image->getFile())->resize($baseImageWidth, $baseImageHeight)->__toString();
				$tmp['label'] = $_image->getLabel();
